...
for k in dimensiones:
    ...
    for _ in range(num):
            ...
            #clusteriamos
            X = lista_coordenadas
            kmeans_coordenadas = MiniBatchKMeans(n_clusters=3, random_state=0).fit(X)
            #print('etiquetas de clustering',kmeans_coordenadas.labels_)
            #print('etiquetas de reales',lista_clases)
            X = lista_coordenadas_val
            kmeans_coordenadas_val = MiniBatchKMeans(n_clusters=2, random_state=0).fit(X)
            #print('etiquetas de clustering',kmeans_coordenadas_val.labels_)
            #print('etiquetas de reales',lista_clases_val)
            
            clases_train=kmeans_coordenadas.labels_ == lista_clases
            
            kmeans_coordenadas_val.labels_= np.where(kmeans_coordenadas_val.labels_ == 0, 3, kmeans_coordenadas_val.labels_)
            kmeans_coordenadas_val.labels_= np.where(kmeans_coordenadas_val.labels_ == 1, 4, kmeans_coordenadas_val.labels_)
            
            clases_val=kmeans_coordenadas_val.labels_ == lista_clases_val
